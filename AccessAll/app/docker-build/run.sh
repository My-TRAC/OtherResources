...
echo " /opt/ConfigurationScripts/setJDBCSinkConnector.sh -c "cigo-jdbc-sink_AccessAll" -k $KAFKA_CONNECT_HOST -s $SCHEMA_REGISTRY_HOST_NAME -n $SINK_TOPICS -m $MYSQL_HOST -d $MYSQL_DATABASE -u $MYSQL_USER -p $MYSQL_PASSWORD -pk \"mytrac_id\" -ac \"true\""
